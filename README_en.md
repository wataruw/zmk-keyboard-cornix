...
Older Cornix/RMK firmwares used a “no SoftDevice (no-SD)” flash layout. If your dongle/board doesn’t work after flashing, either:
...
Original RMK removed the SoftDevice. Pre-v2.3 you needed to restore SoftDevice before flashing `zmk.uf2` (see bootloader/README.md).

Since v2.3 the flash partitions were updated (SD reduced/removed), so you can flash your firmware directly.

For v2.3+ you normally do NOT need reset.uf2 before flashing `cornix_left_default.uf2` and `cornix_right.uf2`. Use reset.uf2 only if:
